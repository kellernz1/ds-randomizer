...
test("real catalog produces deterministic enemies with visibly different models", async () => {
  ...
  assert.ok(
    first.placements.enemies.every((placement) =>
      [
        ...
        "disabled-unmatched-dragon-part",
        "synthetic-linked-dragon-part",
  ...
  assert.ok(
    ["c2300", "c3420", "c3421", "c3430", "c3520", "c3530", "c5351"]
      .includes(hellkiteBridge.targetModelName),
  );
  ...
  assert.ok(titaniteDemonBodies.every(
    (placement) =>
      ...
      ["c3420", "c3421", "c3430", "c3520", "c3530", "c5351"].includes(
...
test("real catalog produces real boss and world-item placements", async () => {
  ...
  assert.equal(asylum.combatPauseFlagId, 11810315);
  assert.equal(asylum.combatReentryFlagId, 11815390);
